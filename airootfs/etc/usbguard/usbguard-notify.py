...
def show_dialog(device_info):
    try:
        ...
        notify("USB Detectado", f"{device_info}\n(Error: {e})")


def notify(title, text):
    subprocess.run(["notify-send", "-u", "normal", title, text])
...
if __name__ == "__main__":
    ...
    if action in ["insert", "present"]:
        info = subprocess.run(["usbguard", "list-devices", "-d"], capture_output=True, text=True)
        device_info = info.stdout.strip()

        if device_id:
            threading.Thread(
                target=show_dialog,
                args=(f"Dispositivo USB conectado:\n\n{device_info}",),
                daemon=True,
            ).start()
