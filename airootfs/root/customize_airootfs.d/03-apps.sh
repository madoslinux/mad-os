...
restrict_wallpaper_desktop_to_sway() {
    ...
    for desktop in "${desktop_files[@]}"; do
        ...
        if grep -q '^OnlyShowIn=' "$desktop"; then
            sed -i 's/^OnlyShowIn=.*/OnlyShowIn=Hyprland;Sway;/' "$desktop"
        elif grep -q '^\[Desktop Entry\]' "$desktop"; then
            sed -i '/^\[Desktop Entry\]/a OnlyShowIn=Hyprland;Sway;' "$desktop"
        fi

        echo "  → Enabled $(basename "$desktop") for Hyprland/Sway"
    done
}
...
setup_wallpaper_assets() {
    ...
    if [[ -d /home/mados ]]; then
        mkdir -p /home/mados/.local/share/mados/wallpapers /home/mados/Pictures/Wallpapers
        ...
        chown -R 1000:1000 /home/mados/.local/share/mados /home/mados/Pictures
    fi
