...
assert_installer_contract() {
    ...
    if ! grep -q '_mount_btrfs_with_retry' "${install_path}/installer/steps.py"; then
        echo "ERROR: Installer contract check failed: steps.py missing btrfs mount retry helper"
        return 1
    fi

    if ! grep -q '"mount", "-t", "btrfs"' "${install_path}/installer/steps.py"; then
        echo "ERROR: Installer contract check failed: steps.py missing explicit btrfs mount type for subvolumes"
        return 1
    fi
...
helper_fn = '''def _mount_btrfs_with_retry(app, root_part, mount_point, retries=6):
...
anchor = '''def _format_partitions_real(boot_part, root_part):
...
if "def _mount_btrfs_with_retry(" not in t and anchor in t:
    t = t.replace(anchor, anchor + helper_fn, 1)
    changed = True
...
if old_finally in t:
    t = t.replace(old_finally, new_finally, 1)
    changed = True
...
PY
        if ! grep -q '_mount_btrfs_with_retry' "${install_path}/installer/steps.py"; then
            echo "ERROR: Failed to harden installer Btrfs mount retry"
            return 1
        fi
        if ! grep -q '"mount", "-t", "btrfs"' "${install_path}/installer/steps.py"; then
            echo "ERROR: Failed to enforce explicit Btrfs mount type for subvolumes"
            return 1
        fi
        echo "  → Hardened installer Btrfs subvolume mount with retry logic"
