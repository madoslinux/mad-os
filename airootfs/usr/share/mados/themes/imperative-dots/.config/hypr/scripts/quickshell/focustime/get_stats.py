...
def main():
    ...
    params_avg = (
        ...
        if app_filter
        else (monday.isoformat(), sunday.isoformat())
    )
    c.execute(
        f"""
    ...
    for i in range(7):
        ...
        week_data.append(
            {"date": d.isoformat(), "day": days_str[i], "total": tot, "is_target": d == target_date}
        )
    ...
    try:
        c.execute(
            f"SELECT hour, SUM(seconds) FROM focus_hourly WHERE log_date = ?{filter_sql} GROUP BY hour",
            params,
        )
        for row in c.fetchall():
            hr, secs = row
    ...
    try:
        c.execute(
            f"SELECT interval_idx, SUM(seconds) FROM focus_intervals WHERE log_date = ?{filter_sql} GROUP BY interval_idx",
            params,
        )
        for row in c.fetchall():
            idx, secs = row
