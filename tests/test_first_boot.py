...
class TestAudioConfiguration(unittest.TestCase):
    ...
    def test_audio_init_script_exists(self):
        ...
        script = os.path.join(REPO_DIR, "airootfs", "usr", "local", "bin", "mados-audio-init.sh")
        self.assertTrue(
            os.path.isfile(script), "mados-audio-init.sh must be pre-installed on live USB"
        )

    def test_audio_init_service_exists(self):
        """mados-audio-init.service must exist on the live USB."""
        service = os.path.join(
            REPO_DIR, "airootfs", "etc", "systemd", "system", "mados-audio-init.service"
        )
        self.assertTrue(
            os.path.isfile(service), "mados-audio-init.service must be pre-installed on live USB"
        )
    ...
    def test_audio_init_service_is_enabled(self):
        ...
        self.assertTrue(
            os.path.islink(symlink), "mados-audio-init.service must be enabled on live USB"
        )

    def test_audio_quality_service_exists(self):
        """mados-audio-quality.service must exist on the live USB."""
        service = os.path.join(
            REPO_DIR, "airootfs", "etc", "systemd", "system", "mados-audio-quality.service"
        )
        self.assertTrue(
            os.path.isfile(service), "mados-audio-quality.service must be pre-installed on live USB"
        )
    ...
    def test_user_audio_quality_service_in_skel(self):
        ...
        self.assertTrue(
            os.path.isfile(skel_svc), "User-level audio quality service must be in /etc/skel"
        )
...
class TestChromiumConfiguration(unittest.TestCase):
    ...
    def test_chromium_flags_exists(self):
        ...
        flags = os.path.join(REPO_DIR, "airootfs", "etc", "chromium-flags.conf")
        self.assertTrue(
            os.path.isfile(flags), "chromium-flags.conf must be pre-installed on live USB"
        )
    ...
    def test_chromium_flags_has_content(self):
        ...
        with open(flags) as f:
            content = f.read()
        self.assertIn("--ozone-platform", content, "Chromium must use Wayland via --ozone-platform")

    def test_chromium_homepage_policy_exists(self):
        """Chromium homepage policy JSON must exist on the live USB."""
        policy = os.path.join(
            REPO_DIR, "airootfs", "etc", "chromium", "policies", "managed", "mados-homepage.json"
        )
        self.assertTrue(
            os.path.isfile(policy), "Chromium homepage policy must be pre-installed on live USB"
        )
    ...
    def test_chromium_homepage_policy_is_valid_json(self):
        ...
        with open(policy) as f:
            parsed = json.load(f)
        self.assertIn("HomepageLocation", parsed, "Chromium policy must contain HomepageLocation")
    ...
    def test_progress_markers_use_8_steps(self):
        ...
        for step, total in markers:
            with self.subTest(step=step):
                self.assertEqual(total, "8", f"PROGRESS {step}/{total} should be {step}/8")
